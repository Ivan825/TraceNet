...
✅ Service skeletons
✅ Demo microservice request flow
✅ Trace ID propagation
✅ Trace span ingestion
✅ PostgreSQL persistence
✅ Trace query service
✅ Diagnostic summary endpoint
✅ API Gateway routing
✅ Auth service
✅ JWT generation
...
TraceNet is a multi-tenant distributed tracing and API failure diagnosis platform built using Spring Boot microservices. It captures request spans across distributed services, persists trace data in PostgreSQL, provides trace querying and diagnostic summaries, generates latency/error analytics, and evaluates alert conditions. The platform uses API Gateway routing, JWT authentication, permission-based RBAC, organization-level data isolation, and secured service APIs.
...
```text
Architected TraceNet, a multi-tenant distributed tracing and API failure diagnosis platform using Spring Boot microservices, API Gateway, JWT authentication, PostgreSQL, and permission-based RBAC.

Built trace ingestion and query services to capture distributed request spans, persist latency/error metadata, and reconstruct full request journeys by trace ID.

Implemented organization-level data isolation using JWT org claims and gateway-propagated context headers to prevent cross-tenant trace access.

Developed analytics and alerting services to calculate service latency, p95 latency, error rates, slow traces, and threshold-based alerts.

Secured platform APIs through API Gateway JWT validation and permission-based authorization for trace ingestion, querying, analytics, and alert management.
```
